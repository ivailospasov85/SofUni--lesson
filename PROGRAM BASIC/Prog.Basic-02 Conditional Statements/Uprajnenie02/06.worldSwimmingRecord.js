// Иван решава да подобри Световния рекорд по плуване на дълги разстояния. Функцията получава: рекордът в секунди,  който Иван трябва да подобри,  
...
// че: съпротивлението на водата го забавя на всеки 15 м. с 12.5 секунди. Когато се изчислява колко пъти Иванчо ще се забави, в резултат на съпротивлението 
...
let worldRecordInSecunds=Number(input[0]);
let distanceMetriks= Number(input[1]);
let timeInSecundForMeturs=Number(input[2]);
let ivanDelaySeconds=Math.floor(distanceMetriks/15)*12.5;
let ivanTime=(distanceMetriks*timeInSecundForMeturs)+ivanDelaySeconds;
if(ivanTime<worldRecordInSecunds){
  console.log (`Yes, he succeeded! The new world record is ${(ivanTime).toFixed(2)} seconds.`)
}else {
    console.log(`No, he failed! He was ${(ivanTime-worldRecordInSecunds).toFixed(2)} seconds slower.`)
}
// console.log(ivanTime)
