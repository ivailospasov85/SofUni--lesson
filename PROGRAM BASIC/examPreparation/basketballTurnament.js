...
function basketballTurnament(input) {
    let index = 0
    ...
    while (command !== "End of tournaments") {
        nameOfTheTurnament = command;
        
        let tournamentCount = Number(input[index])
        index++
        for (let i = 0; i < tournamentCount; i++) {
           
            let teamAScore = input[index]
            index++
            let teamBScore = input[index]
            index++
            counter++
            if (teamAScore > teamBScore) {
                winCounter++
                console.log(`Game ${i+1} of tournament ${nameOfTheTurnament}: win with ${Math.abs(teamAScore - teamBScore)} points.`);
            } else {
                loseCounter++
                console.log(`Game ${i+1} of tournament ${nameOfTheTurnament}: lost with ${Math.abs(teamBScore - teamAScore)} points.`);
            }
        }
        command = input[index]
        index++
    }
    console.log(`${((winCounter / counter) * 100).toFixed(2)}% matches win`)
    console.log(`${((loseCounter / counter) * 100).toFixed(2)}% matches lost`);
